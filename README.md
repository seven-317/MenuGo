...
│   ├── layout.tsx            # 根版面
│   ├── globals.css           # 全域樣式
│   ├── admin/                # 後台：登入／註冊／主選單、餐桌、菜單、入座 QR、接單
│   ├── scan/[token]/         # 顧客掃碼點餐頁
│   ├── auth/callback/        # Supabase Auth OAuth／magic link 回呼
│   └── api/                  # Route Handlers
│       ├── order/            # 顧客送單（RPC create_customer_order）
│       ├── orders/[orderId]/ # 後台接單、標記完成
│       ├── qrcode/           # 產生 QR 圖
│       └── scan/             # 組掃碼網址、依 token 查歷史訂單
├── components/
│   ├── admin/                # 後台表單、即時訂單看板等
│   └── scan/                 # 掃碼點餐、倒數、訂單紀錄 UI
├── lib/                      # Supabase 各環境 client、掃碼 HMAC、admin 工具
├── public/                   # 靜態資源
├── scripts/                  # npm run setup（資料庫／種子）、scan:url 等
├── supabase/                      # schema、migrate_*.sql、RPC、seed_demo_scan.sql
